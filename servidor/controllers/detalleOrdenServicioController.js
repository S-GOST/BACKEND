...
// 2. Obtener UN SOLO detalle por su ID (Usa findById del modelo)
export const obtenerDetalleOrdenPorId = async (req, res) => {
  const { id } = req.params;
  try {
    // Cambiado de findByPk (Sequelize) a findById (SQL)
...
// 3. Obtener detalles filtrando por ID de la Orden (Usa findByOrderId del modelo)
export const obtenerDetallesPorId = async (req, res) => {
  try {
    ...
    // Cambiado de findAll(where) a findByOrderId (SQL)
...
// 5. Actualizar un detalle (Sintaxis corregida para coincidir con el modelo)
export const actualizarDetalleOrden = async (req, res) => {
  try {
    ...
    // Extraemos el ID para no enviarlo en los datos a actualizar
    const { ID_DETALLES_ORDEN_SERVICIO, ...datosParaActualizar } = req.body;

    // El modelo espera: update(id, data)
...
// 6. Eliminar un detalle (Sintaxis corregida para coincidir con el modelo)
