...
export const obtenerTec = async (req, res) => {
    try {
        const Tec = await Tecnico.findAll();
...
export const obtenerTecPorId = async (req, res) => {
    ...
    try {
        ...
        const [rows] = await pool.query('SELECT * FROM tecnicos WHERE ID_TECNICO = ?', [id]);
...
export const crearTec = async (req, res) => {
    try {
        const nuevoTec = await Tecnico.create(req.body);    
...
export const actualizarTec = async (req, res) => {
    ...
    try {
        const tecActualizado = await Tecnico.update(id, req.body);  
