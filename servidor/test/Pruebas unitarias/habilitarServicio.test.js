// test/Pruebas unitarias/habilitarServicio.test.js
...
const { habilitarServicio } = require('../../controllers/serviciosController.js');
...
describe('habilitarServicio', () => {
  ...
  describe('Habilitación exitosa', () => {
    test('Debe devolver 200 y registrar la habilitación correctamente', async () => {
      ...
      Servicio.restore.mockResolvedValue(resultadoRestoreMock);
      logHistory.mockResolvedValue();

      const req = { params: { id }, user: { id_usuario: 3 } };
      const res = mockRes();

      await habilitarServicio(req, res);

      expect(Servicio.restore).toHaveBeenCalledWith(id);
  ...
  describe('Casos no encontrados (404)', () => {
    test('Debe devolver 404 si el servicio no existe (P2025)', async () => {
      const id = '999';
      const notFoundError = new Error('Not found');
      notFoundError.code = 'P2025';

      Servicio.restore.mockRejectedValue(notFoundError);
  ...
  describe('Manejo de errores', () => {
    test('Debe devolver 500 si falla la restauración en la base de datos', async () => {
      ...
      Servicio.restore.mockRejectedValue(dbError);
